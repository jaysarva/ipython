...
class TestLineNumberUpdates(ShellFixture):
    """Test line number updates for all code patterns"""
    
    def test_comprehensive_line_number_updates(self):
        """Test that line numbers are updated for all function types"""
        self.shell.magic_autoreload("2")
        
        # Create initial module with various function patterns
        mod_name, mod_fn = self.new_module("""
            # Line 1
            def regular_func():
                return "regular"
        ...
        # Update module with added lines at the beginning
        ...
        # Verify line numbers were updated (should be +3 for each)
        updated_lines = {
            'regular_func': mod.regular_func.__code__.co_firstlineno,
            'method': mod.TestClass.method.__code__.co_firstlineno,
            'static_func': mod.TestClass.static_func.__code__.co_firstlineno,
        ...
        # Check that line numbers were properly updated
        for func_name in original_lines:
            expected_line = original_lines[func_name] + 3
            actual_line = updated_lines[func_name]
            self.assertEqual(
                actual_line, expected_line,
                f"Line number for {func_name} should be {expected_line} but got {actual_line}"
            )
        
        # Verify functions still work correctly
        self.assertEqual(mod.regular_func(), "regular")
        
        # Test class methods
        test_instance = mod.TestClass()
        self.assertEqual(test_instance.method(), "method")
        self.assertEqual(mod.TestClass.static_func(), "static")
        self.assertEqual(mod.TestClass.class_method(), "class_method")
        self.assertEqual(test_instance.outer_method(), "nested")

    def test_decorated_function_line_updates(self):
        """Test line number updates specifically for decorated functions"""
        self.shell.magic_autoreload("2")
        ...
        # Check that decorated function line number was updated
        # Note: After fixing decorator line number handling, @custom_decorator should be at line 8
        # (originally at line 6, moved down by 2 lines)
        expected_line = 8  # @custom_decorator line after adding 2 lines at the top
        self.assertEqual(updated_line, expected_line,
                        f"decorated_func should be at line {expected_line} after update, got {updated_line}")
        
        # Verify function still works
        self.assertEqual(mod.decorated_func(), "decorated")
