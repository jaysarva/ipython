...
def _patched_extract_tb(tb, limit=None):
    ...
    if _deduperreloader:
        ...
        current_tb = tb
        for entry in result:
            if hasattr(entry, 'filename') and entry.filename == "<string>" and current_tb:
                ...
                if original_filename:
                    ...
                    corrected_entry = traceback.FrameSummary(
                        original_filename, entry.lineno, entry.name, 
                        line=entry.line, locals=getattr(entry, 'locals', None)
                    )
...
def _patched_format_exception(etype, value, tb, limit=None, chain=True):
